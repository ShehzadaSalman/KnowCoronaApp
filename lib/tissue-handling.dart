...
//                                       children: quizHelper.results[index].incorrectAnswers.map((f){
                                              ...
                                              print(
                                                  'Marks from this section: ' +
                                                      tissueHandlingMarks
                                                          .toString());
                                              print(
                                                  'Saving data tissueHandlingMarks...');
                                              saveToSharedPreferences();
                                              print('Data saved!');
                                              final snackBar = SnackBar(
                                                  content: Text(
                                                      'Congratulations! You have Completed the Tissue Handling Section'));
  ...
  void initializeSharedPreferences() async {
    ...
    tissueHandlingCompleted =
        sharedPreferences.getBool('tissueHandlingCompleted');
    tissueHandlingMarks = sharedPreferences.getDouble('tissueHandlingMarks');
    print('tissueHandlingCompleted - from SharedPreferences : ' +
        tissueHandlingCompleted.toString());
    if (tissueHandlingCompleted != null && tissueHandlingCompleted) {
      print('Section already completed');
      print('Marks obtained in Tissue Handling section: ' +
          tissueHandlingMarks.toString());
    }
  }
  ...
  void saveToSharedPreferences() async {
    ...
    await sharedPreferences.setDouble(
        'tissueHandlingMarks', tissueHandlingMarks);
...
class _SurveyCarouselState extends State<SurveyCarousel> {
  ...
  Widget build(BuildContext context) {
    List<SurveyTitle> surveylist = [
      SurveyTitle(context.getString('menu.tissue'), 1, '/tissue'),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          Image(
            image: AssetImage('Images/Group-93.png'),
          ),
          SizedBox(height: 30),
          Text(
            context.getString('menu.tissueIntro'),
            textAlign: TextAlign.center,
            style: TextStyle(
              fontSize: 21,
              fontFamily: 'Seg',
